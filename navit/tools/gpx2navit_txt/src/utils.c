...
double
getDistanceCore(char *p1, char *l1, char *p2, char *l2);
void
checkEllpsUnit(char *unit);
double
checkLengthUnit(char *unit);
int
checkTimeUnit(char *unit);
double
getTimeInterval(char *_t, char *t);
double
getSpeed(double length, double ti, double to_meter, int to_sec);
double
getDistance(double _x, double _y, double x, double y);
// todo void closeShpFiles(shphandles * shps);
// todo void closeDbfFiles(dbfhandles * dbfs);
void *
myMallocRep(size_t size, const char *fileName, int line);
