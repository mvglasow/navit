 ...
 * may have been deallocated. Like map rectangles, items are not meant to be shared between threads: if you need to
 * share items between threads, pass the map and item ID, and have the receiving thread open a map rectangle on that
 * map and receive its own copy of the item. Failure to do so may result in race conditions and hard-to-debug errors.
 *
